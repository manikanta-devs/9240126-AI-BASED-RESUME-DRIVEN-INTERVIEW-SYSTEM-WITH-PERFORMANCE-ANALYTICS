...
def generate_questions():
    ...
    try:
        ...
        logger.info(f"Generating questions: role={role}, difficulty={difficulty}, skills={skills[:10]}, has_resume={'yes' if skills else 'no'}, panel_mode={panel_mode}")
...
def submit_answer():
    ...
    try:
        ...
        interview_service.add_answer(session_id, {
        ...
        return jsonify({
            ...
            'difficulty_adjustment': evaluation.get('difficulty_adjustment', 'maintain'),
            'updated_questions': session['questions'] if (not is_complete and difficulty_adjustment != 'maintain') else None
        }), 200
