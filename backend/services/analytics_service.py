...
class AnalyticsService:
    ...
    def get_study_plan(self) -> dict:
        ...
        weakest_topics = sorted(
            ({'topic': topic, 'avg': round(sum(scores) / len(scores), 1)} for topic, scores in topic_scores.items() if scores),
            key=lambda x: x['avg']
        )[:3]
    ...
    def get_communication_coach(self) -> dict:
        ...
        headline = 'Speak with clarity, confidence, and structure' if avg_clarity >= 60 else 'Build stronger speaking habits'
        summary = (
