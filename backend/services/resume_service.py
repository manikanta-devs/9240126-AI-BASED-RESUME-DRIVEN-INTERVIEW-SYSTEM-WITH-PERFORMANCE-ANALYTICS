...
class ResumeService:
    ...
    SKILL_KEYWORDS = {
        'languages': [
            ...
            'typescript', 'kotlin', 'swift', 'php', 'scala', 'r', 'matlab',
            'perl', 'bash', 'shell', 'dart', 'elixir', 'haskell'
        ],
        'frameworks': [
            'react', 'angular', 'vue', 'django', 'flask', 'fastapi', 'spring',
            'express', 'node', 'nodejs', 'next.js', 'nuxt', 'svelte', 'laravel',
            'rails', 'asp.net', 'tensorflow', 'pytorch', 'keras', 'scikit-learn',
            'pandas', 'numpy', 'fastapi', 'graphql', 'rest', 'redux', 'tailwind'
        ],
        'databases': [
        ...
        'tools': [
            ...
            'vs code', 'intellij', 'xcode', 'android studio', 'jupyter',
            'tableau', 'power bi', 'excel', 'git', 'webpack', 'vite'
        ]
    }

    EDUCATION_KEYWORDS = [
        'bachelor', 'master', 'phd', 'doctorate', 'b.sc', 'm.sc', 'b.tech',
        'm.tech', 'mba', 'b.e', 'm.e', 'degree', 'diploma', 'certification',
    ...
    def _extract_experience(self, text: str, text_lower: str) -> dict:
        ...
        title_keywords = [
            ...
            'specialist', 'director', 'head of', 'vp', 'cto', 'ceo'
        ]
        lines = text.split('\n')
        for line in lines:
            line_lower = line.lower()
    ...
    def _calculate_resume_score(self, skills: dict, education: list, experience: dict, text: str) -> dict:
        ...
        completeness = 0
        if len(text) > 500: completeness += 5
        if len(text) > 1000: completeness += 5
        if 'email' in text.lower() or '@' in text: completeness += 5
        if any(w in text.lower() for w in ['project', 'achievement', 'award']): completeness += 5
        if len(education) > 0: completeness += 5
    ...
    def compare_resume_to_job(self, resume_data: dict, job_description: str) -> dict:
        ...
        return {
            ...
            'summary': self._build_match_summary(total_score, matched_skills, missing_skills, required_years, resume_years)
        }
    ...
    def _build_match_summary(self, score: int, matched_skills: list, missing_skills: list,
                             required_years: int, resume_years: int) -> str:
