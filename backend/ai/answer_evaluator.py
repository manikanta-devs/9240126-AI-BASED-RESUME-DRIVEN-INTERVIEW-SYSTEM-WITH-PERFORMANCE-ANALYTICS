...
class AnswerEvaluator:
    ...
    def evaluate(self, question: dict, answer: str, role: str = 'software_engineer',
                 voice_metrics: dict | None = None, previous_scores: list | None = None) -> dict:
        """Evaluate an answer and return detailed feedback with adaptive fields"""
        if self.gemini.is_available():
            result = self._evaluate_with_gemini(question, answer, role,
        ...
        return self._fallback_evaluation(answer, question=question,
                                          voice_metrics=voice_metrics,
                                          previous_scores=previous_scores)

    def generate_follow_up(self, question: dict, answer: str, evaluation: dict, role: str = 'software_engineer') -> dict:
    ...
    def _evaluate_with_gemini(self, question, answer, role, voice_metrics=None, previous_scores=None):
...
Be fair but honest. If the answer is very short or vague, score accordingly."""
        ...
        if result and isinstance(result, dict):
            return self._validate_evaluation(result, voice_metrics=voice_metrics)
  ...
  "parent_question": "{question_text[:60]}..."
    ...
    def _fallback_follow_up(self, question, evaluation):
        ...
        if question_type == 'behavioral':
            templates = [
                f"What was the measurable outcome of that experience?",
                f"Looking back, what would you do differently?",
                f"How did that change how you approach similar situations now?",
            ]
        elif question_type == 'situational':
            templates = [
                f"What would be the first thing you do in the first 30 minutes?",
                f"How would you prioritize if you had limited time and resources?",
                f"What if the constraint you mentioned was removed — how would your approach change?",
            ]
        else:
            templates = [
                f"Can you go deeper on the technical implementation of {topic}?",
                f"What would be the trade-offs of the approach you described?",
                f"How would you handle edge cases or failure scenarios in your solution?",
                f"Can you walk me through a real example where you applied this?",
            ]
    ...
    def _validate_evaluation(self, result: dict, voice_metrics: dict | None = None) -> dict:
        ...
        overall = clamp(result.get('overall_score', round(tech * 0.4 + clarity * 0.2 + completeness * 0.2 + relevance * 0.2)))
        ...
        voice_delivery_score = clamp(result.get('voice_delivery_score', default_voice_delivery), default=default_voice_delivery)
        ...
        return {
            ...
            'follow_up_prompt': result.get('follow_up_prompt', 'How would you explain that to a non-technical interviewer?'),
    ...
    def _fallback_evaluation(self, answer: str, question: dict = None,
                              voice_metrics: dict | None = None,
                              previous_scores: list | None = None) -> dict:
        ...
        has_example = any(w in lower for w in ['for example', 'for instance', 'in my', 'we built', 'project', 'implementation'])
        has_structure = any(w in lower for w in ['first', 'then', 'finally', 'as a result', 'situation', 'task', 'action', 'result'])
        has_numbers = bool(__import__('re').search(r'\d+%?|\$\d+', answer))
        has_tradeoff = any(w in lower for w in ['trade-off', 'tradeoff', 'downside', 'however', 'on the other hand', 'alternative'])
        ...
        if has_example: tech += 5; completeness += 5
        if has_structure: clarity += 8
        if has_numbers: tech += 5; completeness += 3
        if has_tradeoff: tech += 7
        ...
        live_tips = []
        if not has_example: live_tips.append('Add a specific example from your experience')
        if not has_structure: live_tips.append('Structure your answer with a clear beginning, middle, and end')
        if not has_numbers: live_tips.append('Include metrics or numbers to quantify your impact')
        if filler_word_count > 5: live_tips.append('Reduce filler words for clearer delivery')

        # Strong/weak areas
        strong = []
        weak = []
        if has_example: strong.append('Uses real examples')
        if has_structure: strong.append('Well-structured response')
        if has_numbers: strong.append('Quantified impact')
        if word_count >= 50: strong.append('Good depth')
        if not has_example: weak.append('Needs concrete examples')
        if not has_structure: weak.append('Improve answer structure')
        if word_count < 30: weak.append('Too brief')
        if not has_tradeoff and word_count > 20: weak.append('Discuss trade-offs')
        ...
        return {
            ...
            'voice_feedback': 'Good voice delivery. Keep your pace steady and reduce filler words.' if voice_metrics else 'Focus on pace and reducing filler words.',
