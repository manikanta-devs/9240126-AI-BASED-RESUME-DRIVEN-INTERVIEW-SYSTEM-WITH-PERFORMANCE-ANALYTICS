...
const DIFF_OPTIONS = [
  { value: 'easy', label: 'ðŸŸ¢ Easy', desc: 'Fundamental concepts' },
  { value: 'medium', label: 'ðŸŸ¡ Medium', desc: 'Industry standard level' },
  { value: 'hard', label: 'ðŸ”´ Hard', desc: 'Senior/expert level' },
]
...
export default function InterviewPage() {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      if (nextDiff !== adaptiveDifficulty) {
        ...
        toast(`Difficulty adjusted to ${nextDiff}`, { icon: 'ðŸ“Š' })
      }
  ...
  const handleTimerExpire = () => {
    toast("Time's up! Auto-submitting...", { icon: 'â±ï¸' })
  ...
  const handleRetryAnswer = () => {
    ...
    setPhase(PHASE.INTERVIEWING)
    toast('Retry mode â€” give a better answer!', { icon: 'ðŸ”„' })
  }
      ...
      <div className="max-w-3xl mx-auto space-y-5 animate-in">
        <div className="card">
          ...
          <p className="text-sm text-gray-500 mb-2">
            {resumeData
              ? <span className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500 inline" /> Resume loaded â€” questions will be tailored to your skills.</span>
              : <span className="flex items-center gap-2"><AlertCircle className="w-4 h-4 text-orange-400 inline" /> No resume uploaded â€” using generic role-based questions.</span>
            }
          </p>
  ...
  return (
    <div className="space-y-4 animate-in max-w-7xl mx-auto">
      ...
      <div className="grid lg:grid-cols-[1fr_340px] gap-4">
        ...
        <div className="space-y-4">
          ...
          {(isListening || voiceMetrics) && (
            <div className="flex items-center gap-4 px-4 py-3 rounded-2xl bg-slate-900/80 border border-white/[0.06]">
              <MetricChip label="WPM" value={voiceMetrics?.speaking_pace_wpm || 'â€”'} status={voiceMetrics?.speaking_pace_wpm >= 110 && voiceMetrics?.speaking_pace_wpm <= 170 ? 'good' : voiceMetrics?.speaking_pace_wpm ? 'warn' : 'idle'} />
              <MetricChip label="Fillers" value={voiceMetrics?.filler_count ?? 'â€”'} status={voiceMetrics?.filler_count <= 3 ? 'good' : voiceMetrics?.filler_count ? 'warn' : 'idle'} />
              <MetricChip label="Words" value={voiceMetrics?.word_count || answer.split(/\s+/).filter(Boolean).length || 'â€”'} status="idle" />
              <MetricChip label="Clarity" value={evaluation?.clarity_score ? `${evaluation.clarity_score}` : 'â€”'} status={evaluation?.clarity_score >= 70 ? 'good' : evaluation?.clarity_score ? 'warn' : 'idle'} />
              <MetricChip label="Confidence" value={evaluation?.confidence_score ? `${evaluation.confidence_score}` : 'â€”'} status={evaluation?.confidence_score >= 70 ? 'good' : evaluation?.confidence_score ? 'warn' : 'idle'} />
            </div>
          )}
        ...
        {/* RIGHT COLUMN â€” Live Feedback Panel */}
